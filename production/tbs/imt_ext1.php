...
SELECT
  ...
  CASE WHEN (p.program_id=17) THEN COUNT(p.program_id) ELSE 0 END AS 'col1', -- 'Microcredit/ microfinance
  CASE WHEN (p.program_id=18) THEN COUNT(p.program_id) ELSE 0 END AS 'col2', -- 'Assistance to Pantawid farmer benefeciaries
...
$template = './templates/IMT_EXT1.xlsx';
...
$output_file_name = 'IMT_EXT1_'.date('Y-m-d').'.xlsx';
